...
function initAIEvents() {
	...
	if (pauseAIBtn) {
		pauseAIBtn.addEventListener('click', async () => {
			...
			try {
				const aiStreamSelect = document.getElementById('ai-stream-select');
				const streamId = aiStreamSelect?.value?.trim();
				if (!streamId) {
					if (typeof showToast === 'function') showToast('请先选择直播流', 'error');
					return;
				}

				console.log('⏸️ 暂停AI识别，直播流:', streamId);

				// 禁用按钮，防止重复点击
				pauseAIBtn.disabled = true;
				pauseAIBtn.textContent = '暂停中...';

				const result = await toggleAI('pause', streamId, true);
	...
	if (resumeAIBtn) {
		resumeAIBtn.addEventListener('click', async () => {
			...
			try {
				...
				console.log('▶️ 恢复AI识别，直播流:', streamId);

				// 禁用按钮，防止重复点击
				resumeAIBtn.disabled = true;
				resumeAIBtn.textContent = '恢复中...';

				const result = await toggleAI('resume', streamId, true);
...
// 全局 chart 实例，用于切换流/筛选时销毁旧图表
window._voteChart = null;
window._activityChart = null;
...
/**
 * 获取当前选中的 streamId（复用 dashboard 页的逻辑）
 */
function _getStatisticsStreamId() {
	try {
		const select = document.getElementById('stream-select');
		if (select && select.value) return select.value;
	} catch (e) { /* ignore */ }
	try {
		const select = document.getElementById('votes-stream-select');
		if (select && select.value) return select.value;
	} catch (e) { /* ignore */ }
	return null;
}
 ...
 * 渲染投票分析折线图
 */
function renderVoteAnalysisChart(timeline, canvasId) {
	const canvas = document.getElementById(canvasId || 'vote-analysis-chart');
	if (!canvas) return;
	...
	canvas._chart = window._voteChart = new Chart(canvas, {
		type: 'line',
		data: {
			labels,
			datasets: [
				{
					label: '正方票数',
					data: timeline.map(p => p.leftVotes),
					borderColor: '#2196F3',
				...
				{
					label: '反方票数',
					data: timeline.map(p => p.rightVotes),
					borderColor: '#f44336',
		...
		options: {
			...
			scales: {
				...
				y: { title: { display: true, text: '票数' }, beginAtZero: true },
			},
		},
	});
}
...
function renderUserActivityChart(timeline) {
	...
	window._activityChart = new Chart(canvas, {
		...
		data: {
			...
			datasets: [{
				label: '活跃用户',
				data: timeline.map(p => p.activeUsers),
		...
		options: {
			...
			scales: {
				x: { title: { display: true, text: '时间' } },
				y: { title: { display: true, text: '用户数' }, beginAtZero: true },
			},
		},
	});
}

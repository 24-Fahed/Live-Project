...
function handleWebSocketMessage(message) {
	...
	switch (message.type) {
		...
		case 'votes-updated':
			// 投票数据更新 - 只处理当前选中流的票数
			{
				const votesStreamId = document.getElementById('votes-stream-select')?.value;
				const messageStreamId = message.data.streamId;
				if (!votesStreamId || (messageStreamId && votesStreamId !== messageStreamId)) {
					break;
				}
...
async function loadDashboard() {
	try {
		...
		// 渲染实时投票趋势
		if (data.streamId && typeof renderVoteAnalysisChart === 'function') {
			fetch(`/api/v1/admin/statistics/votes?stream_id=${data.streamId}&time_range=1h`)
				.then(r => r.json())
				.then(result => {
					const timeline = result?.data?.timeline || [];
					if (timeline.length > 0) renderVoteAnalysisChart(timeline, 'votes-chart');
				})
				.catch(() => {});
		}
	} catch (error) {
...
async function loadStatistics() {
	try {
		...
		// 获取投票统计时间线
		const streamId = _getStatisticsStreamId();
		const voteStats = await fetchVotesStatistics('1h', streamId);
		...
		// 渲染图表
		const timeline = (voteStats?.data?.timeline) || (voteStats?.timeline) || [];
		if (timeline.length > 0) {
			renderVoteAnalysisChart(timeline);
			renderUserActivityChart(timeline);
		}
...
async function viewStreamDetail(streamId) {
	...
	// 标记为详情视图，阻止定时刷新覆盖
	container.dataset.detailView = 'true';
	...
	container.innerHTML = `
		<div style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; color: rgba(255,255,255,0.8);">
			...
			<div style="font-size: 13px; opacity: 0.7;">正在获取流 ${streamId} 的详情</div>
		</div>
	`;

	try {
		// 并行请求流列表和 dashboard 数据
		const [streamsRes, dashboardRes] = await Promise.all([
			fetch(`${API_BASE}/streams`),
			fetch(`${API_BASE}/dashboard?stream_id=${streamId}`)
		]);
		...
		// 找到对应的流信息
		const streamList = streamsData.data?.streams || streamsData.streams || streamsData.data?.streams || [];
		const stream = Array.isArray(streamList)
			? streamList.find(s => s.id === streamId)
			: streamList;

		if (!stream) {
			throw new Error('未找到该直播流');
		}

		const dashboard = dashboardData.data || dashboardData;
		renderStreamDetail(container, stream, dashboard);

	} catch (error) {
		console.error('❌ 加载流详情失败:', error);
		container.innerHTML = `
			<div style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; color: rgba(255,255,255,0.8);">
				<div style="font-size: 24px; margin-bottom: 12px;">加载失败</div>
				<div style="font-size: 13px; opacity: 0.7; margin-bottom: 20px;">${error.message}</div>
				<button class="btn btn-sm" style="background: rgba(255,255,255,0.2); color: white; border: 1px solid rgba(255,255,255,0.3); display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px;" onclick="backToOverview()">
					<img src="/static/iconfont/shuju.png" style="width: 14px; height: 14px; filter: brightness(0) invert(1);" alt="">
					返回总览
				</button>
			</div>
		`;
 ...
 * 渲染流详情面板
 */
function renderStreamDetail(container, stream, dashboard) {
	const isLive = dashboard.isLive || false;
	const activeUsers = dashboard.activeUsers || 0;
	const totalVotes = dashboard.totalVotes || 0;
	const debateTopic = dashboard.debateTopic || {};
	const liveStartTime = dashboard.liveStartTime || null;
	const liveDuration = dashboard.liveDuration || 0;

	// 状态颜色
	const statusColor = isLive ? '#27ae60' : '#95a5a6';
	const statusText = isLive ? '直播中' : '未开播';
	const statusBorderColor = isLive ? '#27ae60' : '#dee2e6';
	...
	if (liveDuration > 0) {
		...
		const seconds = Math.floor(liveDuration % 60);
		if (hours > 0) {
	...
	container.innerHTML = `
		<div style="grid-column: 1 / -1;">
			...
			<div style="display: flex; align-items: center; gap: 12px; margin-bottom: 20px;">
				<button
					class="btn btn-sm btn-secondary"
					style="padding: 8px 16px; font-size: 13px; display: flex; align-items: center; gap: 6px; white-space: nowrap;"
			...
			">
				...
				<div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
					<div style="flex: 1;">
						<h3 style="margin: 0 0 6px 0; color: #2c3e50; font-size: 20px; font-weight: 700;">
							${stream.name || 'Unnamed Stream'}
						</h3>
						<div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
							<span style="font-size: 12px; color: #6c757d; background: #f0f0f0; padding: 2px 8px; border-radius: 4px;">
								${stream.type ? stream.type.toUpperCase() : 'UNKNOWN'}
							</span>
							<span style="font-size: 12px; color: #95a5a6;">
								ID: ${stream.id}
							</span>
							${stream.enabled ? '' : '<span style="font-size: 12px; color: #e74c3c; background: #fde8e8; padding: 2px 8px; border-radius: 4px;">已禁用</span>'}
						</div>
					</div>
					<div style="background: ${statusColor}; color: white; padding: 6px 16px; border-radius: 4px; font-size: 13px; font-weight: 600; white-space: nowrap; display: flex; align-items: center; gap: 6px;">
						<span class="iconfont icon-circle" style="font-size: 10px; ${isLive ? 'color: white;' : 'opacity: 0.7;'}"></span>
						${statusText}
					</div>
				</div>
				...
				<!-- 描述 -->
				${stream.description ? `
				<div style="padding: 12px 16px; background: #f8f9fa; border-radius: 6px; border: 1px solid #e9ecef; margin-bottom: 20px;">
					<div style="font-size: 11px; color: #95a5a6; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px;">描述</div>
					<div style="font-size: 14px; color: #495057; line-height: 1.5;">${stream.description}</div>
				</div>
				` : ''}

				<!-- 数据统计 -->
				<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 20px;">
					<div style="text-align: center; padding: 14px 10px; background: #f8f9fa; border-radius: 6px; border: 1px solid #e9ecef;">
						<div style="font-size: 24px; font-weight: 700; color: #3498db; display: flex; align-items: center; justify-content: center; gap: 6px;">
							<img src="/static/iconfont/blue-user.png" style="width: 18px; height: 18px; opacity: 0.8;" alt="">
							${activeUsers}
						</div>
						<div style="font-size: 11px; color: #6c757d; margin-top: 4px;">在线用户</div>
					</div>
					<div style="text-align: center; padding: 14px 10px; background: #f8f9fa; border-radius: 6px; border: 1px solid #e9ecef;">
						<div style="font-size: 24px; font-weight: 700; color: #34495e; display: flex; align-items: center; justify-content: center; gap: 6px;">
							<img src="/static/iconfont/toupiao.png" style="width: 18px; height: 18px; opacity: 0.8;" alt="">
							${totalVotes}
						</div>
						<div style="font-size: 11px; color: #6c757d; margin-top: 4px;">总投票</div>
					</div>
					<div style="text-align: center; padding: 14px 10px; background: #f8f9fa; border-radius: 6px; border: 1px solid #e9ecef;">
						<div style="font-size: 24px; font-weight: 700; color: ${isLive ? '#27ae60' : '#95a5a6'}; display: flex; align-items: center; justify-content: center; gap: 6px;">
							<img src="/static/iconfont/live.png" style="width: 18px; height: 18px; opacity: ${isLive ? '1' : '0.4'};" alt="">
							${durationText}
						</div>
						<div style="font-size: 11px; color: #6c757d; margin-top: 4px;">直播时长</div>
					</div>
					<div style="text-align: center; padding: 14px 10px; background: #f8f9fa; border-radius: 6px; border: 1px solid #e9ecef;">
						<div style="font-size: 24px; font-weight: 700; color: ${isLive ? '#e67e22' : '#95a5a6'}; display: flex; align-items: center; justify-content: center; gap: 6px;">
							<img src="/static/iconfont/gongjigongju.png" style="width: 18px; height: 18px; opacity: ${isLive ? '1' : '0.4'};" alt="">
							${dashboard.aiStatus === 'running' ? '运行中' : '未启动'}
						</div>
						<div style="font-size: 11px; color: #6c757d; margin-top: 4px;">AI 状态</div>
					</div>
				</div>
				...
				<div style="border: 1px solid #e9ecef; border-radius: 8px; overflow: hidden; margin-bottom: 20px;">
					<div style="background: #f0f3ff; padding: 10px 16px; font-size: 13px; font-weight: 600; color: #3498db; display: flex; align-items: center; gap: 6px; border-bottom: 1px solid #e9ecef;">
						<img src="/static/iconfont/shuju.png" style="width: 14px; height: 14px; opacity: 0.7;" alt="">
						辩题信息
					</div>
					<div style="padding: 16px;">
						<div style="font-size: 16px; font-weight: 600; color: #2c3e50; margin-bottom: 12px;">${debateTopic.title}</div>
						${debateTopic.description ? `<div style="font-size: 13px; color: #6c757d; margin-bottom: 14px; line-height: 1.5;">${debateTopic.description}</div>` : ''}
						<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
							<div style="padding: 12px 14px; background: #e8f5e9; border-radius: 6px; border-left: 3px solid #27ae60;">
								<div style="font-size: 11px; color: #27ae60; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px;">正方</div>
								<div style="font-size: 14px; color: #2c3e50; font-weight: 500;">${debateTopic.leftSide || '--'}</div>
							</div>
							<div style="padding: 12px 14px; background: #fde8e8; border-radius: 6px; border-left: 3px solid #e74c3c;">
								<div style="font-size: 11px; color: #e74c3c; font-weight: 600; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px;">反方</div>
								<div style="font-size: 14px; color: #2c3e50; font-weight: 500;">${debateTopic.rightSide || '--'}</div>
							</div>
						</div>
					</div>
				</div>
				` : `
				<div style="padding: 20px; text-align: center; background: #f8f9fa; border-radius: 6px; border: 1px dashed #dee2e6; margin-bottom: 20px;">
					<div style="font-size: 13px; color: #95a5a6;">暂无辩题信息</div>
				</div>
				`}
				...
				<div style="padding: 12px 16px; background: #f8f9fa; border-radius: 6px; border: 1px solid #e9ecef; margin-bottom: 20px;">
					<div style="font-size: 11px; color: #95a5a6; margin-bottom: 4px; text-transform: uppercase; letter-spacing: 0.5px;">直播地址</div>
					<div style="font-size: 13px; color: #3498db; word-break: break-all;">${dashboard.liveStreamUrl}</div>
				</div>
				` : ''}

				<!-- 底部操作按钮 -->
				<div style="display: flex; gap: 10px; padding-top: 16px; border-top: 1px solid #e9ecef;">
					<button
						class="btn btn-sm ${isLive ? 'btn-danger' : 'btn-success'}"
						style="flex: 1; padding: 10px; font-size: 14px; display: flex; align-items: center; justify-content: center; gap: 6px;"
						onclick="controlStreamLive('${stream.id}', ${!isLive})"
					>
						${isLive ? '<span class="iconfont icon-stop" style="font-size: 14px;"></span>停止直播' : '<img src="/static/iconfont/bofang.png" style="width: 14px; height: 14px; filter: brightness(0) invert(1);" alt="">开始直播'}
					</button>
					<button
						class="btn btn-sm btn-secondary"
						style="padding: 10px 20px; font-size: 14px; display: flex; align-items: center; gap: 6px; justify-content: center;"
						onclick="viewStreamDetail('${stream.id}')"
					>
						<img src="/static/iconfont/shuaxin.png" style="width: 14px; height: 14px; opacity: 0.7;" alt="">
						刷新
					</button>
				</div>
			</div>
...
function initMultiLiveFeatures() {
	...
	setInterval(() => {
		const dashboardPage = document.getElementById('dashboard');
		const container = document.getElementById('multi-live-streams-grid');
		if (dashboardPage && dashboardPage.classList.contains('active') && container && !container.dataset.detailView) {
			renderMultiLiveOverview();
		}
	}, 10000);
}

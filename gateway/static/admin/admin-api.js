...
// 📋 说明：当前配置
// - 直接访问本地网关服务器 (http://localhost:8080)
 ...
 * @param {string} streamId - 直播流ID（必填）
 * @param {boolean} notifyUsers - 是否推送通知给用户
 * @returns {Promise<Object|null>}
 */
async function toggleAI(action, streamId, notifyUsers = true) {
	if (!streamId) {
		console.error('❌ toggleAI: streamId 是必填参数');
		return null;
	}
 ...
 * @returns {Promise<Object|null>}
 */
async function fetchVotesStatistics(timeRange = '1h', streamId = null) {
	let url = `/api/v1/admin/statistics/votes?time_range=${timeRange}`;

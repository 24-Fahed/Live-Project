...
async function loadStreamsForJudges() {
	try {
		const result = await apiRequest('/api/v1/admin/streams');
...
async function loadJudgesDataForStream(streamId) {
	try {
		const result = await apiRequest(`/api/v1/admin/judges?stream_id=${streamId}`);
		const judges = result?.judges || [];
		if (judges.length > 0) {
			judgesData = judges;
		}
		console.log('📝 加载评委数据:', judgesData);
		updateJudgesUI();
	} catch (error) {
...
async function saveJudgesData() {
	...
	try {
		await apiRequest('/api/v1/admin/judges', {
			method: 'POST',
